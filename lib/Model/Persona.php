...
class Persona implements ModelInterface, ArrayAccess
{
    ...
    public function setNombre($nombre)
    {
        if (!is_null($nombre) && (mb_strlen($nombre) > 30)) {
            throw new \InvalidArgumentException('invalid length for $nombre when calling Persona., must be smaller than or equal to 30.');
        }
    ...
    {
        if (!is_null($dependencia) && (mb_strlen($dependencia) > 120)) {
            throw new \InvalidArgumentException('invalid length for $dependencia when calling Persona., must be smaller than or equal to 30.');
        }
    ...
    public function setDispocisionesId($dispocisiones_id)
    {
        if (!is_null($dispocisiones_id) && (mb_strlen($dispocisiones_id) > 30)) {
            throw new \InvalidArgumentException('invalid length for $dispocisiones_id when calling Persona., must be smaller than or equal to 30.');
        }
    ...
    public function setCurpOk($curp_ok)
    {
        if (!is_null($curp_ok) && (mb_strlen($curp_ok) > 1)) {
            throw new \InvalidArgumentException('invalid length for $curp_ok when calling Persona., must be smaller than or equal to 1.');
        }
        if (!is_null($curp_ok) && (mb_strlen($curp_ok) < 0)) {
            throw new \InvalidArgumentException('invalid length for $curp_ok when calling Persona., must be bigger than or equal to 0.');
        }
